...
import { HotspotBusiness, HotspotPlan, Voucher, Customer, ActiveSession, PaymentRequest, WhatsAppMessageLog, SaaSPlan } from '../types';
import { 
...
export default function OwnerDashboard({
  ...
  const handleCreateSingleVoucher = (e: React.FormEvent) => {
    ...
    if (singleVCustomerPhone) {
      ...
      onAddMessageLog({
        ...
        recipientPhone: singleVCustomerPhone,
        messageType: 'voucher',
        content: logContent,
        status: 'Delivered',
  ...
  const handleResendVoucherWhatsApp = (v: Voucher) => {
    if (!v.customerPhone) {
      alert("No WhatsApp phone details mapped on this voucher code. Edit custom details or copy first!");
      return;
    }
    const recName = v.customerName || 'Valued Client';
    const msgbody = `Hello ${recName}\n\nYour Wi-Fi passcode was resent.\n\nPlan: ${v.planName}\nPIN Code Code: ${v.code}\n\nThank you for choosing ${business.businessName}!`;
    
    onAddMessageLog({
      id: `msg_resend_${Date.now()}`,
      recipientName: recName,
      recipientPhone: v.customerPhone,
      messageType: 'voucher',
      content: msgbody,
      status: 'Delivered',
      timestamp: new Date().toISOString(),
    ...
    <div id="owner-dashboard" className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-4 shadow-sm">
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Active Sessions</p>
          <div className="flex items-baseline gap-1.5 mt-1.5">
            <h3 className="text-xl font-extrabold text-slate-900">{sessions.length} Clients</h3>
            <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
          </div>
          <p className="text-[10.5px] text-slate-500 mt-2">Starlink concurrent load: <strong className="font-bold underline">Medium</strong></p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-emerald-950 to-brand-900 text-brand-100 rounded-2xl p-5 border border-brand-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-4 text-xs font-mono">
              ...
              <div className="h-8 w-px bg-brand-700/50 self-center" />
              <div>
                ...
                {filteredVouchers.map((v) => (
                  <div key={v.id} className="border border-slate-100 hover:border-brand-200/60 p-3 rounded-xl bg-slate-50/50 hover:bg-slate-50/10 transition-colors flex justify-between items-center text-xs">
                    ...
                    <div className="text-right flex items-center gap-2">
                      ...
                      {/* Quick resale resend or deactivate actions */}
                      <button
                        onClick={() => handleResendVoucherWhatsApp(v)}
                        title="Resend Code to subscriber via simulated WhatsApp API"
        ...
        <div className="bg-white border border-slate-200/85 rounded-2xl p-6 shadow-sm">
          <div className="border-b border-slate-100 pb-3 mb-5">
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Review clients transaction screenshots or uploaded references. Click APPROVE to automatically spawn voucher PINs and dispatch instantly over WhatsApp!
            </p>
          </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <span className="px-2.5 py-0.5 bg-brand-500 text-brand-950 rounded-full text-[10px] font-bold uppercase tracking-wider">
                  Starlink Gateway Status
                </span>
                <h3 className="text-lg font-extrabold mt-1">Satellite connection is healthy - Active concurrent sessions</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-lg">
                  This interface maps local clients currently hooked to your Starlink/MikroTik hotspots. Review speeds, mac identities, and raw download counts.
                </p>
              </div>

              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex gap-4 text-center text-xs font-mono">
                <div>
                  <span className="text-slate-400 text-[10px] block uppercase">Network Load</span>
                  <strong className="text-white text-sm">34.8 MBps</strong>
                </div>
                <div className="w-px h-8 bg-slate-700" />
                <div>
                  <span className="text-slate-400 text-[10px] block uppercase">Avg Latency</span>
                  <span className="text-emerald-400 text-sm font-bold">22 ms</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/85 rounded-2xl overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse font-medium text-slate-700">
                <thead>
                  <tr className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100">
                    <th className="p-4">Consumer Account</th>
                    <th className="p-4">IP Address</th>
                    <th className="p-4">MAC Address</th>
                    <th className="p-4">Device Model</th>
                    <th className="p-4">Active Voucher Used</th>
                    <th className="p-4 text-right">Data Exerted</th>
                    <th className="p-4 text-right">Internet Speed load</th>
                    <th className="p-4 text-center">Power Controls</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {sessions.map((sess) => (
                    <tr key={sess.id} className="hover:bg-slate-50/50">
                      <td className="p-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-extrabold text-slate-800">{sess.customerName}</span>
                        </div>
                      </td>
                      <td className="p-4 text-slate-600 font-mono">{sess.ipAddress}</td>
                      <td className="p-4 text-slate-500 font-mono">{sess.macAddress}</td>
                      <td className="p-4 text-slate-600">{sess.deviceType}</td>
                      <td className="p-4"><code className="bg-slate-100 px-1.5 py-0.5 rounded text-[11px] font-mono text-slate-700 font-bold">{sess.voucherCode}</code></td>
                      <td className="p-4 text-right font-extrabold text-slate-800">{sess.dataUsedGb} GB</td>
                      <td className="p-4 text-right text-brand-600 font-bold font-mono">
                        ⬇ {sess.downloadSpeedMbps} Mbps / ⬆ {sess.uploadSpeedMbps} Mbps
                      </td>
                      <td className="p-4 text-center">
                        <button
                          onClick={() => {
                            onDisconnectSession(sess.id);
                            triggerAlert(`Disconnected device session ID: ${sess.id} from local Sat node.`);
                          }}
                          className="px-2 py-1 bg-rose-50 text-rose-600 hover:bg-rose-100 rounded text-[10.5px] font-bold transition-colors"
                        >
                          Disconnect 🚫
                        </button>
                      </td>
      ...
      {/* Tab: WhatsApp log history & provider options */}
      {activeTab === 'whatsapp' && (
        <div className="space-y-6">
          <div className="bg-white border border-slate-200/85 rounded-2xl p-6 shadow-sm">
            <h4 className="text-sm font-extrabold text-slate-800 uppercase tracking-tight mb-2">WhatsApp Provider Integrations (Abstraction layer)</h4>
            <p className="text-xs text-slate-500 mb-4">
              Select which API gateway forwards messages. Perfect for NaijaReceipt-style automation rules without banking interruptions.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pb-4 border-b border-slate-100">
              {['Meta Cloud API', 'Twilio', 'Termii', 'UltraMsg'].map((provName) => {
                const isSel = business.whatsappProvider === provName;
                return (
                  <div
                    key={provName}
                    onClick={() => {
                      const updated = { ...business, whatsappProvider: provName as any };
                      onUpdateBusiness(updated);
                      triggerAlert(`Swapped target WhatsApp Provider to ${provName}`);
                    }}
                    className={`border-2 rounded-xl p-3 text-center cursor-pointer hover:border-brand-500 transition-all ${
                      isSel ? 'border-brand-600 bg-brand-50/40 shadow-sm' : 'border-slate-150'
                    }`}
                  >
                    <span className="text-xl mb-1 block">💬</span>
                    <strong className="text-xs text-slate-800 block">{provName}</strong>
                    <span className="text-[9px] text-slate-400 font-bold block mt-1 uppercase">{isSel ? 'Active Provider' : 'Click to Swap'}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">WhatsApp Gateway secret Credentials</label>
                <input
                  type="password"
                  value={business.whatsappApiKey || 'termii_key_192801'}
                  onChange={(e) => {
                    const upBusiness = { ...business, whatsappApiKey: e.target.value };
                    onUpdateBusiness(upBusiness);
                  }}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">WhatsApp Customer alert phone</label>
                <input
                  type="text"
                  value={business.phone}
                  onChange={(e) => {
                    const upBusiness = { ...business, phone: e.target.value, whatsapp: e.target.value };
                    onUpdateBusiness(upBusiness);
                  }}
          ...
          {/* Historical Logs */}
          <div className="bg-white border border-slate-200/85 rounded-2xl p-6 shadow-sm">
            <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider mb-3">WhatsApp system delivery message logs</h4>
            <div className="space-y-3">
              {messageLogs.map((log) => (
                <div key={log.id} className="border border-slate-150 rounded-xl p-3.5 bg-slate-50/50 hover:bg-slate-50/50 text-xs">
                  <div className="flex justify-between items-center mb-1.5 pb-1 border-b border-slate-100/30">
                    <div>
                      <strong className="text-slate-800">{log.recipientName} ({log.recipientPhone})</strong>
                      <span className="text-slate-400 text-[10px] ml-2">Type: {log.messageType}</span>
                    </div>
                  ...
                  <p className="font-mono text-[10.5px] text-slate-600 bg-white border border-slate-200 rounded-lg p-2.5 leading-normal whitespace-pre-wrap">
                    {log.content}
                  </p>
                </div>
              ))}
